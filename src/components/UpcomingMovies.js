...
const UpcomingMovies = () => {
  ...
  useEffect(() => {
    const fetchUpcomingMovies = async () => {
      try {
        ...
        console.log('Upcoming Movies with videos:', moviesWithVideos);
      } catch (error) {
  ...
  if (isLoading) {
    // Render a placeholder or loading indicator while fetching data
    return (
  ...
  return (
    <View style={{ marginBottom: 10, borderRadius: 10, overflow: 'hidden',marginTop:20 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, marginBottom: 10 }}>
        <Text style={{ fontSize: 20, fontWeight: 'bold', color: COLORS.gold }}>Upcoming Movies</Text>
        <TouchableOpacity
      ...
      <FlatList
        ...
        contentContainerStyle={{ paddingHorizontal: 16 }} // Added horizontal padding for the entire list
        style={{ marginHorizontal: -5 }} // Added negative margin to create space between movie cards
      />
    </View>
  );
};
...
const SCard = ({ item, openVideo }) => {
  ...
  return (
    <View>
    <TouchableOpacity
      style={{ alignItems: 'center', borderWidth: 3, borderColor: COLORS.gold, borderRadius: 10, marginBottom: 5, marginLeft: 5, marginRight: 5,marginTop:3 }} // Added horizontal margin for each card
